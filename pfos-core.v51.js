...
var PFOSEngine = (() => {
  ...
  function _ibcWLAnnualCredit(policy, yearNum, currentCV, currentPUA) {
    ...
    const dividend2 = dividendBase * (p.dividendRate || 0.02);
    const puaCVAdded = p.dividendOption === "pua" ? dividend2 * 0.95 : 0;
    const nonPUADividend = p.dividendOption !== "pua" ? dividend2 : 0;
    return {
      cvDelta: premiumToCV + guaranteedGrowth + puaCVAdded,
      puaDelta: puaCVAdded,
      dividend: dividend2,
      nonPUADividend,
      breakdown: { premiumToCV, guaranteedGrowth, dividend: dividend2, puaCVAdded, efficiency }
    };
  }
...
All debts are cleared by month ${r.months} (${yr(r.months)} yr). The policy loans are then repaid with the freed minimums and fully cleared by month ${r.fullFreeMonth} (${yr(r.fullFreeMonth)} yr), after which those minimums cascade toward your other goals.`;
  ...
  // src/engine/calculators/ui.ts
  var ui_exports = {};
  __export(ui_exports, {
    calcAdvisorTeaser: () => calcAdvisorTeaser,
    calcDisclaim: () => calcDisclaim,
    calcDisclosure: () => calcDisclosure,
    calcShareBtn: () => calcShareBtn,
    calcShareCTA: () => calcShareCTA,
    calcStoryBox: () => calcStoryBox,
    calcToggleHTML: () => calcToggleHTML
  });
  function calcStoryBox(icon, text, color) {
    color = color || "#5B9BFF";
    return '<div style="padding:14px;background:rgba(91,155,255,.03);border-left:3px solid ' + color + ';border-radius:0 4px 4px 0;margin-bottom:12px;font-size:13px;color:var(--wh);line-height:1.8">' + icon + " " + text + "</div>";
  }
  function calcToggleHTML(prefix, calcFn) {
    var rerender = calcFn ? "if(typeof " + calcFn + '===\\"function\\")' + calcFn + "()" : "";
    return '<div style="display:flex;gap:6px;margin-bottom:12px"><button id="' + prefix + `_forMe" class="active" onclick="this.classList.add('active');this.style.background='rgba(91,155,255,.15)';this.style.color='#5B9BFF';var o=document.getElementById('` + prefix + "_forOther');o.classList.remove('active');o.style.background='transparent';o.style.color='var(--mu)';" + rerender.replace(/"/g, "'") + '" style="flex:1;padding:8px 12px;border:1px solid rgba(91,155,255,.35);border-radius:4px;background:rgba(91,155,255,.15);color:#5B9BFF;font-family:var(--fh);font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;cursor:pointer;transition:.2s">\u2713 FOR ME</button><button id="' + prefix + `_forOther" onclick="this.classList.add('active');this.style.background='rgba(249,115,22,.15)';this.style.color='var(--or)';var o=document.getElementById('` + prefix + "_forMe');o.classList.remove('active');o.style.background='transparent';o.style.color='var(--mu)';" + rerender.replace(/"/g, "'") + '" style="flex:1;padding:8px 12px;border:1px solid rgba(249,115,22,.25);border-radius:4px;background:transparent;color:var(--mu);font-family:var(--fh);font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;cursor:pointer;transition:.2s">FOR SOMEONE ELSE</button></div>';
  }
  function calcDisclosure(text) {
    return '<div style="margin-top:10px;font-size:12px;color:var(--mu);font-style:italic;line-height:1.6">' + text + "</div>";
  }
  function calcDisclaim(text) {
    return '<div style="font-size:11px;color:var(--mu);line-height:1.5;margin-top:12px;padding-top:10px;border-top:1px solid rgba(0,68,191,.08)">\u2696\uFE0F ' + text + "</div>";
  }
  function calcShareCTA() {
    return `<div style="display:flex;gap:8px;margin-top:14px"><button onclick="if(navigator.share)navigator.share({title:document.getElementById('insCalcTitle').textContent,text:'Check out this financial tool from PFOS'}).catch(function(){});else{navigator.clipboard.writeText(window.location.href);this.textContent='\u2713 Copied!';var b=this;setTimeout(function(){b.textContent='\u{1F4CB} SHARE RESULTS'},2000)}" style="flex:1;padding:10px;border:1px solid rgba(91,155,255,.3);border-radius:4px;background:rgba(91,155,255,.08);color:#5B9BFF;font-family:var(--fh);font-size:12px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;cursor:pointer;transition:.2s">\u{1F4CB} SHARE RESULTS</button><button onclick="if(typeof openConsultModal==='function')openConsultModal();else _cpToast('Contact your advisor to discuss these results.')" style="flex:1;padding:10px;border:1px solid rgba(249,115,22,.3);border-radius:4px;background:rgba(249,115,22,.08);color:var(--or);font-family:var(--fh);font-size:12px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;cursor:pointer;transition:.2s">\u{1F4AC} DISCUSS WITH ADVISOR</button></div>`;
  }
  function calcShareBtn() {
    return `<div style="display:flex;gap:8px;margin-top:14px"><button onclick="if(navigator.share)navigator.share({title:document.getElementById('insCalcTitle').textContent,text:'Check out this financial tool from PFOS'}).catch(function(){});else{navigator.clipboard.writeText(window.location.href);this.textContent='\u2713 Copied!';var b=this;setTimeout(function(){b.textContent='\u{1F4CB} SHARE RESULTS'},2000)}" style="flex:1;padding:10px;border:1px solid rgba(91,155,255,.3);border-radius:4px;background:rgba(91,155,255,.08);color:#5B9BFF;font-family:var(--fh);font-size:12px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;cursor:pointer;transition:.2s">\u{1F4CB} SHARE RESULTS</button></div>`;
  }
  function calcAdvisorTeaser(numbers) {
    return '<div style="padding:12px;background:linear-gradient(135deg,rgba(249,115,22,.06),rgba(249,115,22,.02));border:1px solid rgba(249,115,22,.2);border-radius:4px;margin-top:10px;text-align:center"><div style="font-size:13px;color:var(--wh);line-height:1.6;margin-bottom:8px">' + numbers + `</div><button onclick="if(typeof openConsultModal==='function')openConsultModal();else _cpToast('Contact your advisor.')" style="padding:10px 20px;background:var(--or);border:none;border-radius:4px;color:#fff;font-family:var(--fh);font-size:12px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;cursor:pointer">REQUEST FREE CONSULTATION \u2192</button></div>`;
  }
  ...
  __export(calculators_exports, {
    ...
    ESA_ANNUAL_LIMIT_2026: () => ESA_ANNUAL_LIMIT_2026,
    ...
    HSA_FAMILY_2026: () => HSA_FAMILY_2026,
    HSA_SELF_2026: () => HSA_SELF_2026,
    K401_EMPLOYEE_2026: () => K401_EMPLOYEE_2026,
    MC_DRAW_RATES: () => MC_DRAW_RATES,
    MC_INFL: () => MC_INFL,
    MC_SEED: () => MC_SEED,
    MC_SIMS: () => MC_SIMS,
    MC_TARGET_SUCCESS: () => MC_TARGET_SUCCESS,
    MONEY_PROFILES: () => MONEY_PROFILES,
    ORDINARY_OFFSET_LIMIT: () => ORDINARY_OFFSET_LIMIT,
    ROTH_IRA_2026: () => ROTH_IRA_2026,
    ROTH_IRA_ANNUAL_LIMIT_2026: () => ROTH_IRA_ANNUAL_LIMIT_2026,
    ROTH_MAGI_TOP_2026: () => ROTH_MAGI_TOP_2026,
    SEC127_ANNUAL_TAXFREE: () => SEC127_ANNUAL_TAXFREE,
    SS_RELIABILITY: () => SS_RELIABILITY,
    STD_DEDUCTION_2026_SINGLE: () => STD_DEDUCTION_2026_SINGLE,
    ...
    rentVsBuySimple: () => rentVsBuySimple,
    rental: () => rental,
    retirementFI: () => retirementFI,
    rmd: () => rmd,
    rothConv: () => rothConv,
    sandwichGen: () => sandwichGen,
    savConsistency: () => savConsistency,
  ...
  function insNeeds(input) {
    ...
    const debtPayoff2 = totalDebt;
    const eduFund = deps * 1e5;
    const childExp = deps > 0 ? monthlyExp * 0.3 * kidYrs * 12 : 0;
    const finalExp = 25e3;
    const efBuffer = monthlyExp * 6;
    let totalLifeNeed = incomeReplace + debtPayoff2 + eduFund + childExp + finalExp + efBuffer;
    if (spouseInc > 0) totalLifeNeed = Math.max(totalLifeNeed - spouseInc * 5, income * 5 + debtPayoff2 + finalExp);
  ...
  function rentVsBuySimple(input) {
    const rent = pf(input.rent) || 1800, price = pf(input.price), stay = pi(input.stay) || 7, downPct = pf(input.downPct);
    const creditScore = pi(input.creditScore) || 720, rentIncrease = pf(input.rentIncrease);
    const down = price * downPct, loan = price - down;
    const rate = creditScore >= 740 ? 0.06 : creditScore >= 670 ? 0.065 : 0.075, mr = rate / 12, n = 360;
    const pmt = amortPayment(loan, mr, n);
    const propTax = price * 0.012 / 12, ins = price * 5e-3 / 12, maint = price * 0.01 / 12;
    const buyMonthly = Math.round(pmt + propTax + ins + maint);
    const appreciation = 0.035;
    let totalRent = 0;
    for (let y = 0; y < stay; y++) totalRent += rent * Math.pow(1 + rentIncrease, y) * 12;
    const totalBuy = buyMonthly * stay * 12 + down;
    let bal = loan;
    for (let m = 0; m < stay * 12; m++) {
      const ip = bal * mr;
      bal = Math.max(0, bal - (pmt - ip));
    }
    const homeVal = price * Math.pow(1 + appreciation, stay);
    const sellingCosts = Math.round(homeVal * 0.055), equity = homeVal - bal - sellingCosts;
    const netBuyCost = totalBuy - equity;
    const winner = netBuyCost < totalRent ? "BUY" : "RENT", savings = Math.abs(totalRent - netBuyCost);
    let breakEven = 0;
    for (let yr = 1; yr <= 30; yr++) {
      let tr = 0;
      for (let y2 = 0; y2 < yr; y2++) tr += rent * Math.pow(1 + rentIncrease, y2) * 12;
      const tb = buyMonthly * yr * 12 + down;
      const hv = price * Math.pow(1 + appreciation, yr);
      let b2 = loan;
      for (let m2 = 0; m2 < yr * 12; m2++) {
        b2 = Math.max(0, b2 - (pmt - b2 * mr));
      }
      const eq = hv - Math.max(0, b2);
      if (tb - eq < tr) {
        breakEven = yr;
        break;
      }
    }
    const rent5 = Math.round(rent * Math.pow(1 + rentIncrease, Math.min(4, stay - 1)));
    const rent10 = Math.round(rent * Math.pow(1 + rentIncrease, Math.min(9, stay - 1)));
    return { down, loan, rate, pmt, propTax, ins, maint, buyMonthly, totalRent, totalBuy, bal, homeVal, sellingCosts, equity, netBuyCost, winner, savings, breakEven, rent5, rent10, appreciation };
  }
  var STD_DEDUCTION_2026_SINGLE = 16100;
  function buyVsRentFull(input) {
    const price = pf(input.price), downPct = pf(input.downPct), down = Math.round(price * downPct);
    const rate = pf(input.rate), rm = rate / 12, termYrs = pi(input.termYrs) || 30, n = termYrs * 12;
    const propTaxRate = pf(input.propTaxRate), insYr = pf(input.insYr) || 1800, hoa = pf(input.hoa), maintPct = pf(input.maintPct), closingPct = pf(input.closingPct);
    const appr = pf(input.appr), rent = pf(input.rent) || 1800, rentInc = pf(input.rentInc), rIns = pf(input.rIns) || 25;
    const investRet = pf(input.investRet), stay = pi(input.stay) || 7, taxBrack = pf(input.taxBrack), homeInsInfl = pf(input.homeInsInfl);
    const loan = price - down, closingCosts = Math.round(price * closingPct);
    const pmt = amortPayment(loan, rm, n);
    const timeline = [];
    let bal = loan, cumBuyCost = down + closingCosts, cumRentCost = 0, cumTaxSaving = 0, investPort = down + closingCosts;
    for (let yr = 1; yr <= stay; yr++) {
      const homeVal = price * Math.pow(1 + appr, yr);
      const yrMortgage = pmt * 12;
      const yrPropTax = Math.round(homeVal * propTaxRate);
      const yrIns = Math.round(insYr * Math.pow(1 + homeInsInfl, yr - 1));
      const yrHOA = Math.round(hoa * 12 * Math.pow(1.03, yr - 1));
      const yrMaint = Math.round(homeVal * maintPct);
      let yrInterest = 0;
      for (let m = 0; m < 12; m++) {
        const ip = bal * rm;
        yrInterest += ip;
      ...
      const taxSaving = Math.round(Math.max(0, yrInterest - STD_DEDUCTION_2026_SINGLE) * taxBrack);
      cumTaxSaving += taxSaving;
      cumBuyCost += yrMortgage + yrPropTax + yrIns + yrHOA + yrMaint - taxSaving;
      const yrRent = Math.round(rent * Math.pow(1 + rentInc, yr - 1) * 12);
      const yrRIns = Math.round(rIns * 12);
      cumRentCost += yrRent + yrRIns;
      investPort *= 1 + investRet;
      const sellingCosts = Math.round(homeVal * 0.055), equity = homeVal - bal - sellingCosts;
      ...
      timeline.push({
        ...
        cumBuy: Math.round(cumBuyCost),
        netBuy: Math.round(netBuyCost),
        cumRent: Math.round(cumRentCost),
        investPort: Math.round(investPort),
        rentMonthly: Math.round(rent * Math.pow(1 + rentInc, yr - 1)),
        buyMonthly: Math.round(pmt + yrPropTax / 12 + yrIns / 12 + hoa + yrMaint / 12),
        taxSaved: Math.round(cumTaxSaving)
      });
    }
    const final = timeline[timeline.length - 1];
    const winner = final.netBuy < final.cumRent ? "BUY" : "RENT", savings = Math.abs(final.netBuy - final.cumRent);
    let breakEven = 0;
    for (let i = 0; i < timeline.length; i++) {
      if (timeline[i].netBuy < timeline[i].cumRent) {
        breakEven = timeline[i].yr;
        break;
      }
    }
    return { down, loan, closingCosts, pmt, timeline, winner, savings, breakEven };
  }

  // src/engine/calculators/relocation.ts
  function relocation(input) {
    const curSal = pf(input.curSal) || 1e5;
    const newSal = pf(input.newSal) || curSal;
    const partnerSal = pf(input.partnerSal) || 0;
    const curHousing = pf(input.curHousing) || 1800;
    const newHousing = pf(input.newHousing) || 1500;
    const familySz = pi(input.familySz) || 2;
    const homeVal = pf(input.homeVal) || 0;
    const reloPkg = pf(input.reloPkg) || 0;
    const moveType = input.moveType || "job";
    const curInc = pf(input.curInc) || 0, curProp = pf(input.curProp) || 1;
    const newInc = pf(input.newInc) || 0, newProp = pf(input.newProp) || 1;
    const totalIncome = curSal + partnerSal;
    let totalNewIncome = newSal + partnerSal;
    if (moveType === "remote") totalNewIncome = totalIncome;
    const filing = partnerSal > 0 ? "married" : "single";
    const curTax = taxEstimate({ w2: totalIncome, filing });
    const newTax = taxEstimate({ w2: totalNewIncome, filing });
    const curFedTax = curTax.bracketBreakdown.reduce((s, b) => s + b.tax, 0);
    const newFedTax = newTax.bracketBreakdown.reduce((s, b) => s + b.tax, 0);
    const curStateTax = Math.round(totalIncome * curInc / 100);
    const newStateTax = Math.round(totalNewIncome * newInc / 100);
    const fica = curTax.fica;
    const ficaNew = newTax.fica;
    const curTakeHome = totalIncome - curFedTax - curStateTax - fica;
    const newTakeHome = totalNewIncome - newFedTax - newStateTax - ficaNew;
    const takeHomeDiff = newTakeHome - curTakeHome;
    const housingDiffAnnual = (curHousing - newHousing) * 12;
    const curPropTax = Math.round(homeVal * curProp / 100);
    const newPropTaxEst = Math.round(homeVal * newProp / 100);
    const housingRatio = curHousing > 0 ? newHousing / curHousing : 1;
    const foodRatio = 0.85 + housingRatio * 0.15;
    const transRatio = 0.9 + housingRatio * 0.1;
    const utilRatio = 0.8 + housingRatio * 0.2;
    const childcareRatio = 0.7 + housingRatio * 0.3;
    const curFood = Math.round(600 * familySz);
    const newFood = Math.round(curFood * foodRatio);
    const curTrans = 400, newTrans = Math.round(curTrans * transRatio);
    const curUtil = 200, newUtil = Math.round(curUtil * utilRatio);
    const curChildcare = familySz > 2 ? 1200 : 0;
    const newChildcare = Math.round(curChildcare * childcareRatio);
    const curMonthlyCOL = curHousing + curFood + curTrans + curUtil + curChildcare;
    const newMonthlyCOL = Math.round(newHousing + newFood + newTrans + newUtil + newChildcare);
    const colDiff = (newMonthlyCOL - curMonthlyCOL) * 12;
    const colRatio = curMonthlyCOL > 0 ? newMonthlyCOL / curMonthlyCOL : 1;
    const equivSalary = Math.round(curSal * colRatio);
    const movers = Math.round(3e3 + familySz * 800);
    const travelCost = Math.round(500 + familySz * 200);
    const tempHousing = Math.round(newHousing * 1.5);
    const sellCost = homeVal > 0 ? Math.round(homeVal * 0.06) : 0;
    const buyCost = homeVal > 0 ? Math.round(homeVal * colRatio * 0.03) : 0;
    const leaseBreak = homeVal === 0 ? Math.round(curHousing * 2) : 0;
    const deposits = Math.round(newHousing + 500);
    const totalMovingCost = movers + travelCost + tempHousing + sellCost + buyCost + leaseBreak + deposits;
    const margRate2 = newTax.marginalRate / 100;
    const reloAfterTax = Math.round(reloPkg * (1 - margRate2));
    const outOfPocket = Math.max(0, totalMovingCost - reloAfterTax);
    const netAnnualImpact = takeHomeDiff - colDiff;
    const breakEvenMonths = outOfPocket > 0 && netAnnualImpact > 0 ? Math.round(outOfPocket / (netAnnualImpact / 12)) : 0;
    const fiveYearData = [];
    let cumNet = 0;
    for (let yr = 1; yr <= 5; yr++) {
      cumNet += netAnnualImpact;
      const netAfterMove = cumNet - outOfPocket;
      fiveYearData.push({ yr, annual: netAnnualImpact, cumulative: Math.round(netAfterMove) });
    }
    const moveWins = netAnnualImpact > 0;
    return {
      totalIncome,
      totalNewIncome,
      curFedTax,
  ...
  // src/engine/calculators/education.ts
  function schoolCalc(s, label, monthlyRate) {
    const cost = pf(s.cost) || 3e4, aid = pf(s.aid) || 1e4, salary = pf(s.salary) || 5e4;
    const net = cost - aid, total4 = net * 4;
    const pmt = total4 > 0 ? Math.round(amortPayment(total4, monthlyRate, 120)) : 0;
    const totalPaid = pmt * 120, interest = totalPaid - total4;
    return { name: s.name && String(s.name) || label, cost, aid, net, total4, salary, pmt, totalPaid, interest };
  }
  function collegeCompare(input) {
    const monthlyRate = (pf(input.rate) || 5.5) / 100 / 12;
    const schools = [
      schoolCalc(input.a || {}, "School A", monthlyRate),
      schoolCalc(input.b || {}, "School B", monthlyRate)
    ];
    const diff = Math.abs(schools[0].total4 - schools[1].total4);
    const cheaper = schools[0].total4 <= schools[1].total4 ? schools[0] : schools[1];
    const pricier = schools[0].total4 > schools[1].total4 ? schools[0] : schools[1];
    return { schools, diff, cheaper, pricier };
  }
  function educationGap(input) {
    const kids = input.children || [];
    const children = kids.map(function(child) {
      const age = pf(child.age) || 0;
      const inf = (child.costInflation == null ? 6 : pf(child.costInflation)) / 100;
      const yearsUntilCollege = Math.max(0, 18 - age);
      const mos = yearsUntilCollege * 12;
      const annualCost = pf(child.goal) || 1e5;
      const projCost = annualCost * Math.pow(1 + inf, yearsUntilCollege);
      let projSav = 0;
      (child.accounts || []).forEach(function(ac) {
        const gross = pf(ac.grossReturn) || 7;
        const fund = pf(ac.fundFee) || 0;
        const adv = pf(ac.advFee) || 0;
        const acNet = (gross - fund - adv) / 100;
        const emr = acNet / 12;
        const acBal = pf(ac.balance) || 0;
        const acContrib = pf(ac.contrib) || 0;
        projSav += yearsUntilCollege > 0 ? emr > 0 ? acBal * Math.pow(1 + emr, mos) + acContrib * ((Math.pow(1 + emr, mos) - 1) / emr) : acBal + acContrib * mos : acBal + acContrib;
      });
      const gap = Math.max(0, projCost - projSav);
      return { name: child.name && String(child.name) || "", age, yearsUntilCollege, projCost, projSav, gap };
    });
    const totalCost = children.reduce(function(s, r) {
      return s + r.projCost;
    }, 0);
    return {
      children,
      totalNeed: totalCost,
      totalCost,
      totalSavings: children.reduce(function(s, r) {
        return s + r.projSav;
      }, 0),
      totalGap: children.reduce(function(s, r) {
        return s + r.gap;
      }, 0)
    };
  }
  var ESA_ANNUAL_LIMIT_2026 = 2e3;
  var ROTH_IRA_ANNUAL_LIMIT_2026 = 7500;
  function growFV(mo, yrs, rate) {
    let b = 0;
    for (let m = 0; m < yrs * 12; m++) b = (b + mo) * (1 + rate / 12);
    return Math.round(b);
  }
  function collegeSavings(input) {
    const monthly = pf(input.monthly) || 250;
    const years = Math.max(1, pi(input.years) || 1);
    const annual = monthly * 12;
    const esaCap = ESA_ANNUAL_LIMIT_2026 / 12;
    const rothCap = ROTH_IRA_ANNUAL_LIMIT_2026 / 12;
    const proj = [
      growFV(monthly, years, 0.07),
      // 529 — tax-free growth
      growFV(Math.min(monthly, esaCap), years, 0.07),
      // Coverdell ESA ($2,000/yr cap)
      growFV(monthly, years, 0.065),
      // UTMA/UGMA — after kiddie-tax drag
      growFV(monthly * 0.65, years, 0.055),
      // IUL — ~65% to CV after COI
      growFV(monthly * 0.55, years, 0.045),
      // Whole Life / IBC — ~55% to CV early
      growFV(monthly * 0.4, years, 0.03),
      // Gerber Grow-Up — very low CV
      growFV(Math.min(monthly, rothCap), years, 0.07)
      // Roth IRA (parent) — $7,500/yr cap
    ];
    const stateTaxSaved = input.stateDed ? Math.round(annual * 0.05 * years) : 0;
    return { proj, stateTaxSaved };
  }

  // src/engine/calculators/debt.ts
  function simulateOrder(order, extra, minDefault, minFloor, maxMonths) {
    const cards = order.map(function(d) {
      const mn = Math.max(pf(d.min) || minDefault, minFloor);
      return { name: d.name && String(d.name) || "Card", bal: pf(d.balance) || 0, rate: (pf(d.rate) || 0) / 100, min: mn, oMin: mn };
    });
    let months = 0, totalInt = 0, freed = 0;
    const wins = [], schedule = [];
    while (cards.some(function(c) {
      return c.bal > 0;
    }) && months < maxMonths) {
      months++;
      let moInt = 0;
      cards.forEach(function(c) {
        if (c.bal <= 0) return;
        const ip = c.bal * c.rate / 12;
        moInt += ip;
        totalInt += ip;
        c.bal += ip;
        const pay = Math.min(c.bal, c.min);
        c.bal -= pay;
        if (c.bal < 0.01) {
          c.bal = 0;
          freed += c.oMin;
      ...
      for (let i = 0; i < cards.length; i++) {
        if (cards[i].bal <= 0) continue;
        const pay = Math.min(cards[i].bal, extraLeft);
        cards[i].bal -= pay;
        extraLeft -= pay;
        if (cards[i].bal < 0.01) {
          cards[i].bal = 0;
          freed += cards[i].oMin;
          wins.push({ name: cards[i].name, month: months });
        }
        if (extraLeft <= 0) break;
      }
      if (months <= 12 || months % 6 === 0 || !cards.some(function(c) {
        return c.bal > 0;
      })) {
        schedule.push({ mo: months, totalBal: Math.round(cards.reduce(function(t, c) {
          return t + Math.max(0, c.bal);
        }, 0)), moInt: Math.round(moInt) });
      }
    }
    return { months, interest: Math.round(totalInt), wins, schedule };
  }
  ...
  function debtPayoff(debts, extra, opts) {
    const ex = pf(extra) || 0;
    const minDefault = opts && opts.minDefault != null ? opts.minDefault : 25;
    const minFloor = opts && opts.minFloor != null ? opts.minFloor : 10;
    const maxMonths = opts && opts.maxMonths != null ? opts.maxMonths : 600;
    const ds = debts || [];
    const minOnly = simulateOrder(ds.slice().sort(function() {
      return 0;
    }), ex, minDefault, minFloor, maxMonths);
    const avalanche = simulateOrder(ds.slice().sort(function(a, b) {
      return (pf(b.rate) || 0) - (pf(a.rate) || 0);
    }), ex, minDefault, minFloor, maxMonths);
    const snowball = simulateOrder(ds.slice().sort(function(a, b) {
      return (pf(a.balance) || 0) - (pf(b.balance) || 0);
    }), ex, minDefault, minFloor, maxMonths);
    let hybridBest = null;
    permute(ds).forEach(function(p) {
      const r = simulateOrder(p, ex, minDefault, minFloor, maxMonths);
      if (!hybridBest || r.months < hybridBest.months || r.months === hybridBest.months && r.interest < hybridBest.interest) hybridBest = r;
    });
    const bestPure = avalanche.months < snowball.months ? avalanche : avalanche.months > snowball.months ? snowball : avalanche.interest <= snowball.interest ? avalanche : snowball;
    const hb = hybridBest;
    const beats = hb != null && (hb.months < bestPure.months || hb.months === bestPure.months && hb.interest < bestPure.interest);
    const hybrid = beats && hb != null ? hb : { months: 0, interest: 0, wins: [], schedule: [], _naReason: "No ordering finishes faster or cheaper than avalanche/snowball" };
    return { minOnly, avalanche, snowball, hybrid };
  }
  function planSim(debtArr, extraPmt) {
    const ds = debtArr.map(function(d, i) {
      const mn = Math.max(pf(d.min) || 0, 10);
      return { name: d.name && String(d.name) || "Debt " + (i + 1), bal: pf(d.balance) || 0, rate: (pf(d.rate) || 0) / 100 / 12, min: mn, oMin: mn, paid: false, paidMonth: 0 };
    });
    let months = 0, totalInt = 0, freed = 0;
    const order = [];
    while (ds.some(function(d) {
      return d.bal > 0.01;
    }) && months < 600) {
      months++;
      ...
      let extraLeft = extraPmt + freed;
      for (let i = 0; i < ds.length; i++) {
        if (ds[i].bal > 0) {
          const pay = Math.min(extraLeft, ds[i].bal);
          ds[i].bal -= pay;
          extraLeft -= pay;
          if (ds[i].bal < 0.01) {
            ds[i].bal = 0;
    ...
    return { months, interest: Math.round(totalInt), intSaved: 0, order, perDebt: ds };
  }
  function debtPlan(debts, extra, method) {
    if (!debts || !debts.length) return { months: 0, interest: 0, intSaved: 0, order: [], perDebt: [] };
    const totalMin = debts.reduce(function(s, d) {
      return s + (pf(d.min) || 0);
    }, 0);
    if (totalMin <= 0) return { months: 0, interest: 0, intSaved: 0, order: [], perDebt: [] };
    const ex = pf(extra) || 0;
    if (method === "hybrid") {
      const avalSorted = debts.slice().sort(function(a, b) {
        return (pf(b.rate) || 0) - (pf(a.rate) || 0);
      });
      const snowSorted = debts.slice().sort(function(a, b) {
        return (pf(a.balance) || 0) - (pf(b.balance) || 0);
      });
      const avalSim = planSim(avalSorted, ex), snowSim = planSim(snowSorted, ex);
      let best = null;
      permute(debts).forEach(function(p) {
        const s = planSim(p, ex);
        if (!best || s.months < best.months || s.months === best.months && s.interest < best.interest) {
          best = s;
          best._hybridOrder = p.map(function(d) {
            return d.name || "";
          });
        }
      });
      const bestPure = avalSim.months < snowSim.months ? avalSim : avalSim.months > snowSim.months ? snowSim : avalSim.interest <= snowSim.interest ? avalSim : snowSim;
      const bb = best;
      if (bb == null) return { months: 0, interest: 0, intSaved: 0, order: [], perDebt: [] };
      const beats = bb.months < bestPure.months || bb.months === bestPure.months && bb.interest < bestPure.interest;
      if (!beats) return { months: 0, interest: 0, intSaved: 0, order: [], perDebt: [], _naReason: "No ordering finishes faster or cheaper than avalanche/snowball" };
      const baseSim = planSim(avalSorted, 0);
      bb.intSaved = Math.max(0, baseSim.interest - bb.interest);
      return bb;
    }
    const sorted = debts.slice();
    if (method === "avalanche") sorted.sort(function(a, b) {
      return (pf(b.rate) || 0) - (pf(a.rate) || 0);
    });
    else if (method === "snowball") sorted.sort(function(a, b) {
      return (pf(a.balance) || 0) - (pf(b.balance) || 0);
    });
    const base = planSim(sorted, 0), opt = planSim(sorted, ex);
    opt.intSaved = Math.max(0, base.interest - opt.interest);
    return opt;
  }
  function consolidationPlan(debts, creditScore) {
    if (!debts || !debts.length) return null;
    const totalBal = debts.reduce(function(t, d) {
      return t + (pf(d.balance) || 0);
    }, 0);
    const totalMin = debts.reduce(function(t, d) {
      return t + (pf(d.min) || 0);
    }, 0);
    if (totalBal <= 0 || totalMin <= 0) return null;
    const cs = pi(creditScore) || 700;
    const conRate = cs >= 740 ? 7 : cs >= 670 ? 12 : cs >= 580 ? 18 : 24;
    const avgRate = debts.reduce(function(t, d) {
      return t + (pf(d.rate) || 0) * (pf(d.balance) || 0);
    }, 0) / totalBal;
    if (conRate >= avgRate) return { eligible: false, reason: "Estimated consolidation rate (" + conRate + "%) is not lower than your average rate (" + avgRate.toFixed(1) + "%)." };
    const monthlyRate = conRate / 100 / 12;
    let bal = totalBal, months = 0, totalInt = 0;
    const pmt = totalMin;
    ...
    return { eligible: true, months, interest: Math.round(totalInt), rate: conRate, payment: Math.round(pmt), totalBal, creditScore: cs };
  }
  function simulateDF(debt, mr, payment, maxMonths) {
    let bal = debt, months = 0, totalInt = 0;
    const yearData = [];
    while (bal > 0 && months < maxMonths) {
      months++;
      const ip = bal * mr;
      totalInt += ip;
      bal = Math.max(0, bal - (payment - ip));
      if (months % 12 === 0 || bal <= 0) yearData.push({ mo: months, bal: Math.round(bal), int: Math.round(totalInt) });
    }
    return { months, totalInt: Math.round(totalInt), yearData };
  }
  function debtFreePreview(input) {
    const debt = pf(input.debt) || 0;
    const rate = pf(input.rate) || 0.12;
    const pmt = pf(input.pmt) || 800;
    const extraPmt = pf(input.extraPmt) || 0;
    const mr = rate / 12;
    const base = simulateDF(debt, mr, pmt + extraPmt, 600);
    const extras = [100, 200, 500].map(function(ex) {
      const r = simulateDF(debt, mr, pmt + extraPmt + ex, 600);
      return { extra: ex, months: r.months, saved: Math.round(base.totalInt - r.totalInt), faster: base.months - r.months };
    });
    return { base, extras };
  }
  function creditSim(input) {
    const score = pi(input.score) || 695;
    const action = input.action || "payoff";
    const amtLabel = input.amtLabel != null ? String(input.amtLabel) : "";
    const changes = {
      payoff: { lo: 20, hi: 45, dir: 1, desc: "Paying off " + amtLabel + " reduces utilization significantly. Payment history (35%) stays intact, utilization (30%) improves.", why: "Utilization is 30% of score", icon: "\u{1F4B3}", timeframe: "1-2 billing cycles" },
      newcard: { lo: 5, hi: 15, dir: -1, desc: "Hard inquiry drops score 5-10 points. New account lowers average age. But higher total limit helps long-term.", why: "Hard inquiry + new account age", icon: "\u{1F195}", timeframe: "Dips 1-3 months, recovers in 6-12" },
      miss: { lo: 60, hi: 100, dir: -1, desc: "Payment history is 35% of your score \u2014 the single biggest factor. A 30-day late payment is devastating and stays 7 years.", why: "Payment history = 35% of score", icon: "\u{1F6A8}", timeframe: "Impact lasts 7 YEARS" },
      close: { lo: 10, hi: 30, dir: -1, desc: "Closing reduces total available credit, increasing utilization. Also reduces average account age.", why: "Utilization + account age", icon: "\u274C", timeframe: "Immediate, slow recovery" },
      mortgage: { lo: 5, hi: 10, dir: -1, desc: "Hard inquiry \u2014 temporary small dip. Rate shopping within 14-45 days counts as single inquiry.", why: "Hard inquiry", icon: "\u{1F3E0}", timeframe: "Recovers in 2-3 months" },
      limit: { lo: 5, hi: 20, dir: 1, desc: "Soft pull (if preapproved) or hard pull. Lower utilization ratio without reducing debt.", why: "Lower utilization ratio", icon: "\u{1F4C8}", timeframe: "1-2 billing cycles" }
    };
    const c = changes[action] || changes.payoff;
    const newLo = Math.max(300, Math.min(850, score + c.lo * c.dir));
    const newHi = Math.max(300, Math.min(850, score + c.hi * c.dir));
    return { lo: c.lo, hi: c.hi, dir: c.dir, desc: c.desc, why: c.why, icon: c.icon, timeframe: c.timeframe, newLo, newHi };
  }

  // src/engine/calculators/health-score.ts
  function sustainableMonthly(balance, yearsInRet, realAnnualReturn) {
    if (!(balance > 0)) return 0;
    const n = (yearsInRet || 25) * 12;
    const r = (realAnnualReturn != null ? realAnnualReturn : 0.01) / 12;
    if (Math.abs(r) < 1e-4) return balance / n;
    return balance * (r / (1 - Math.pow(1 + r, -n)));
  }
  function healthScore(ctx) {
    const income = pf(ctx.income) || 0, expenses = pf(ctx.expenses) || 0;
    const efTarget = pf(ctx.efTarget) || 0, efBal = pf(ctx.efBal) || 0, totalDebt = pf(ctx.totalDebt) || 0;
    const allocations = ctx.allocations || [], policies = ctx.policies || [], sim = ctx.sim || null;
    const af = sim ? sim.allocFuture || null : null;
    if (income <= 0) return 0;
    function willBeFunded(type) {
      if (allocations.some(function(a) {
        return a.type === type && (pf(a.amount) || 0) > 0;
      })) return true;
      if (af) return af.some(function(f) {
        return f.type === type && (pf(f.futureAmount) || 0) > 0;
      });
      return false;
    }
    let score = 0;
    const efPct = efTarget > 0 ? Math.min(1, efBal / efTarget) : 0;
    const efAlloc = allocations.find(function(a) {
      return a.type === "ef" && (pf(a.amount) || 0) > 0;
    });
    if (efPct >= 1) score += 15;
    else if (efAlloc && efAlloc._completed) score += Math.max(12, Math.round(efPct * 12) + 3);
    else if (efAlloc) score += 12;
    else score += Math.round(efPct * 15);
    if (totalDebt <= 0) score += 15;
    else {
      const dAlloc = allocations.find(function(a) {
        return a.type === "debt" && (pf(a.amount) || 0) > 0;
      });
      if (dAlloc) {
        const dfm = sim ? pf(sim.debtFreeMonth) || 0 : 0;
        if (dfm > 0 && dfm <= 24) score += 13;
        else if (dfm > 0 && dfm <= 48) score += 10;
        else if (dfm > 0 && dfm <= 72) score += 7;
        else score += 5;
      } else {
        const dRatio = income > 0 ? totalDebt / (income * 12) : 5;
        if (dRatio < 0.5) score += 8;
        else if (dRatio < 1) score += 4;
        else score += 2;
      }
      score += dAlloc ? 2 : 0;
    }
    ...
    if (sr >= 20) score += 15;
    else if (sr >= 15) score += 12;
    else if (sr >= 10) score += 8;
    else if (sr >= 5) score += 4;
    const protTypes = {};
    policies.forEach(function(p) {
      const t = (p.type || "").toLowerCase();
      if (t.indexOf("life") >= 0 || t.indexOf("term") >= 0) protTypes.life = true;
      if (t.indexOf("disab") >= 0) protTypes.disability = true;
      if (t.indexOf("health") >= 0) protTypes.health = true;
    });
    const planAddresses = { life: false, disability: false, health: false };
    allocations.forEach(function(a) {
      if (a.type !== "protection" && a.type !== "custom") return;
      const n = (a.customName || "").toLowerCase();
      if (n.indexOf("term life") >= 0 || n.indexOf("life insur") >= 0) planAddresses.life = true;
      if (n.indexOf("disab") >= 0) planAddresses.disability = true;
      if (n.indexOf("health") >= 0 || n.indexOf("medical") >= 0) planAddresses.health = true;
    });
    score += protTypes.life ? 5 : planAddresses.life ? 3 : 0;
    score += protTypes.disability ? 5 : planAddresses.disability ? 3 : 0;
    score += protTypes.health ? 5 : planAddresses.health ? 3 : 0;
    if (willBeFunded("roth")) score += 3;
    if (willBeFunded("401k")) score += 3;
    if (willBeFunded("iul") || willBeFunded("wl")) score += 4;
    const householdType = ctx.householdType || "", hasPartner = !!ctx.hasPartner;
    const projRetValue = sim ? pf(sim.projRetValue) || 0 : 0;
    if (sim && projRetValue > 0) {
      let atY2 = function(arr, y) {
        return arr[y] != null ? arr[y] : arr[arr.length - 1] || 0;
      };
      var atY = atY2;
      if (hasPartner && (householdType === "joint" || householdType === "hybrid") && sim.retirementByOwner) {
        const age = pf(ctx.age) || 35, spAge = pf(ctx.spAge) || 35;
        const retAge = pf(ctx.retirementAge) || 65, spRetAge = pf(ctx.spRetAge) || 65, lifeExp = pf(ctx.lifeExp) || 90;
        const myYr = Math.max(1, Math.min(30, retAge - age)), spYr = Math.max(1, Math.min(30, spRetAge - spAge)), jtYr = Math.max(myYr, spYr);
        const selfArr = sim.retirementByOwner.self || [], spArr = sim.retirementByOwner.spouse || [], jtArr = sim.retirementByOwner.joint || [];
        const myBal = atY2(selfArr, myYr) + atY2(jtArr, jtYr) * 0.5, spBal = atY2(spArr, spYr) + atY2(jtArr, jtYr) * 0.5;
        const myInc = sustainableMonthly(myBal, Math.max(1, lifeExp - retAge || 25), 0.01);
        const spInc = sustainableMonthly(spBal, Math.max(1, lifeExp - spRetAge || 25), 0.01);
        const myTgt = pf(ctx.myRetTarget) || 0, spTgt = pf(ctx.spRetTarget) || 0;
        const myPct = myTgt > 0 ? Math.min(1, myInc / myTgt) : 0, spPct = spTgt > 0 ? Math.min(1, spInc / spTgt) : 0;
        score += Math.round(Math.min(myPct, spPct) * 15);
      } else {
        const projInc = sustainableMonthly(projRetValue, 25, 0.01);
        const myTgt = pf(ctx.myRetTarget) || 0;
        const pct = myTgt > 0 ? Math.min(1, projInc / myTgt) : 0;
        score += Math.round(pct * 15);
      }
    } else {
      const retAllocs = allocations.filter(function(a) {
        return (a.type === "roth" || a.type === "401k" || a.type === "iul" || a.type === "wl" || a.type === "ibc" || a.type === "hsa" || a.type === "trad_ira" || a.type === "solo401k" || a.type === "roth401k" || a.type === "invest") && (pf(a.amount) || 0) > 0;
      });
      const retTotal = retAllocs.reduce(function(t, a) {
        return t + (pf(a.amount) || 0);
      }, 0);
      const retPct = income > 0 ? retTotal / income * 100 : 0;
      if (retPct >= 20) score += 15;
      else if (retPct >= 15) score += 12;
      else if (retPct >= 10) score += 8;
      else if (retPct >= 5) score += 4;
      else if (retPct > 0) score += 2;
    }
    const activeTypes = [];
    allocations.forEach(function(a, i) {
      let amt = pf(a.amount) || 0;
      if (af && af[i] && (pf(af[i].futureAmount) || 0) > 0) amt = Math.max(amt, pf(af[i].futureAmount) || 0);
      if (amt > 0 && activeTypes.indexOf(a.type || "") < 0) activeTypes.push(a.type || "");
    });
    const uniqueTypes = activeTypes.length;
    if (uniqueTypes >= 5) score += 15;
    else if (uniqueTypes >= 4) score += 12;
    else if (uniqueTypes >= 3) score += 8;
    else if (uniqueTypes >= 2) score += 4;
    else if (uniqueTypes >= 1) score += 2;
    return Math.min(100, score);
  }

  // src/engine/calculators/lifeevent.ts
  var CC_MONTHLY = { daycare: 1200, nanny: 2500, family: 0, sahp: 0 };
  function babyCost(input) {
    const cc = input.cc || "daycare", oopMax = pf(input.oopMax) || 5e3, delivery = input.delivery || "vaginal";
    const sahp = !!input.sahp, mo529 = pf(input.mo529) || 100, inc = pf(input.inc) || 0;
    const ccMonthly = CC_MONTHLY[cc] != null ? CC_MONTHLY[cc] : 1200;
    const deliveryCost = delivery === "csection" ? Math.min(oopMax, 15e3) : Math.min(oopMax, 8e3);
    let prenatal = Math.min(oopMax - deliveryCost, 2e3);
    prenatal = Math.max(0, prenatal);
    const preBirth = deliveryCost + prenatal;
    const yr1 = { childcare: ccMonthly * 12, diapers: 1200, formula: sahp ? 600 : 1800, clothing: 800, gear: 2500, medical: 1500, utilities: 600, insurance: 1200 };
    const yr1Total = preBirth + Object.values(yr1).reduce(function(s, v) {
      return s + v;
    }, 0);
    const monthly = Math.round((yr1Total - preBirth) / 12);
    const childTC = 2200;
    const depCareFSA = cc === "daycare" || cc === "nanny" ? 7500 : 0;
    const depCareSavings = Math.round(depCareFSA * 0.22);
    const totalCredits = childTC + depCareSavings;
    const sahpIncomeLoss = sahp ? Math.round(inc * 0.4) * 12 : 0;
    let fv529 = 0;
    for (let m = 0; m < 18 * 12; m++) fv529 = (fv529 + mo529) * (1 + 0.07 / 12);
    const lifeInsNeed = Math.round(inc * 12 * 18);
    return { ccMonthly, deliveryCost, prenatal, preBirth, yr1, yr1Total, monthly, childTC, depCareFSA, depCareSavings, totalCredits, sahpIncomeLoss, fv529, lifeInsNeed };
  }
  function jobChange(input) {
    const salA = pf(input.salA) || 75e3, bonusA = pf(input.bonusA) || 0, matchA = pf(input.matchA) || 0.03, healthA = pf(input.healthA) || 200, commA = pf(input.commA) || 300, ptoA = pi(input.ptoA) || 15;
    const salB = pf(input.salB) || 82e3, bonusB = pf(input.bonusB) || 5e3, matchB = pf(input.matchB) || 0.06, healthB = pf(input.healthB) || 0, commB = pf(input.commB) || 100, ptoB = pi(input.ptoB) || 20;
    const matchValA = Math.round(salA * matchA), matchValB = Math.round(salB * matchB);
    const ptoValA = Math.round(salA / 260 * ptoA), ptoValB = Math.round(salB / 260 * ptoB);
    const costA = Math.round((healthA + commA) * 12), costB = Math.round((healthB + commB) * 12);
    const totalA = salA + bonusA + matchValA + ptoValA - costA, totalB = salB + bonusB + matchValB + ptoValB - costB;
    const diff = totalB - totalA, winner = diff > 0 ? "NEW OFFER" : "CURRENT JOB";
    return { matchValA, matchValB, ptoValA, ptoValB, costA, costB, totalA, totalB, diff, winner };
  }
  function divorce(input) {
    const incA = pf(input.incA) || 0, incB = pf(input.incB) || 0, assets = pf(input.assets) || 25e4, debts = pf(input.debts) || 8e4, kids = pi(input.kids) || 0, housing = pf(input.housing) || 1800;
    const netAssets = Math.round((assets - debts) / 2);
    const totalInc = incA + incB;
    const newHousingA = Math.round(Math.max(housing * 0.7, incA * 0.3));
    const newHousingB = Math.round(Math.max(housing * 0.6, incB * 0.3));
    const healthIns = 450;
    const childcareAdd = kids > 0 ? Math.round(kids * 400) : 0;
    const expA = newHousingA + healthIns + 1200 + (kids > 0 ? childcareAdd : 0);
    const expB = newHousingB + healthIns + 1e3;
    let csAmount = 0;
    if (kids > 0 && Math.abs(incA - incB) > 500) {
      const higherInc = Math.max(incA, incB);
      csAmount = Math.round(higherInc * 0.17 * kids);
      csAmount = Math.min(csAmount, Math.round(higherInc * 0.35));
    }
    const payerIsA = incA > incB;
    const netA = payerIsA ? incA - csAmount : incA + csAmount;
    const netB = payerIsA ? incB + csAmount : incB - csAmount;
    const gapA = Math.max(0, expA - netA), gapB = Math.max(0, expB - netB);
    return { netAssets, totalInc, newHousingA, newHousingB, healthIns, childcareAdd, expA, expB, csAmount, payerIsA, netA, netB, gapA, gapB };
  }
  function sandwichGen(input) {
    const parentCost = pf(input.parentCost) || 0, childCost = pf(input.childCost) || 0, income = pf(input.income) || 6e3, ownExp = pf(input.ownExp) || 3e3;
    const childYrs = pi(input.childYrs) || 10, parentYrs = pi(input.parentYrs) || 8;
    const totalExp = parentCost + childCost + ownExp;
    const surplus = income - totalExp;
    const caregivingPct = income > 0 ? Math.round((parentCost + childCost) / income * 100) : 0;
    const retSavings = Math.max(0, surplus), retLost = Math.max(0, -surplus);
    const totalParentCost = parentCost * parentYrs * 12, totalChildCost = childCost * childYrs * 12;
    const overlapYrs = Math.min(childYrs, parentYrs);
    let retImpact = 0;
    for (let m = 0; m < overlapYrs * 12; m++) retImpact = (retImpact + retLost) * (1 + 0.07 / 12);
    return { totalExp, surplus, caregivingPct, retSavings, retLost, totalParentCost, totalChildCost, overlapYrs, retImpact };
  }

  // src/engine/calculators/scoring.ts
  function financialAge(input) {
    const age = pi(input.age) || 0, sav = pf(input.sav) || 0, debt = pf(input.debt) || 0;
    const ef = pf(input.ef) || 0, ret = pf(input.ret) || 0, budget = !!input.budget;
    let fAge = age;
    if (sav > 500) fAge -= 3;
    else if (sav < 100) fAge += 5;
    if (debt > 1e5) fAge += 10;
    else if (debt > 5e4) fAge += 5;
    else if (debt < 5e3) fAge -= 3;
    if (ef >= 6) fAge -= 5;
    else if (ef < 1) fAge += 5;
    if (ret > age * 5e3) fAge -= 5;
    else if (ret < age * 1e3) fAge += 5;
    if (budget) fAge -= 2;
    else fAge += 3;
    fAge = Math.max(18, Math.round(fAge));
    const diff = fAge - age;
    return { fAge, diff };
  }
  var MONEY_PROFILES = {
    saver: { emoji: "\u{1F3E6}", title: "The Saver", color: "var(--gr)", desc: "Security-driven. You build buffers, avoid debt, and sleep well knowing your finances are solid.", strength: "Strong financial foundation, low stress, prepared for any emergency.", weakness: "May miss growth opportunities by being too conservative. Cash sitting idle loses to inflation over time.", action: "Consider shifting 20-30% of excess savings into diversified investments. Your safety net is strong \u2014 now let your money work harder." },
    spender: { emoji: "\u{1F389}", title: "The Spender", color: "var(--or)", desc: "Experience-driven. You value enjoyment and believe money is meant to enhance life, not just sit in an account.", strength: "Rich experiences, strong relationships, enjoys the present. Often the most generous personality.", weakness: "Risk of debt accumulation, lifestyle inflation, and under-saving for retirement. May regret later.", action: "Set up automatic savings (at least 20%) BEFORE you spend. Pay future-you first, then enjoy the rest guilt-free." },
    optimizer: { emoji: "\u{1F4CA}", title: "The Optimizer", color: "#5B9BFF", desc: "Strategy-driven. You analyze every decision, maximize returns, and love finding the most efficient path.", strength: "Efficient money management, strong investment returns, tax-optimized strategies. Usually highest net worth.", weakness: "May over-analyze and miss time-sensitive opportunities. Analysis paralysis. Can stress over small amounts.", action: "Set decision time limits. For purchases under $100, decide in 5 minutes. Your instincts are already well-trained." },
    avoider: { emoji: "\u{1F648}", title: "The Avoider", color: "var(--pu)", desc: "Anxiety-driven. Finances feel overwhelming, so you prefer not to engage. Not lazy \u2014 just overwhelmed by complexity.", strength: "Once engaged, often makes rapid progress because the fear was worse than reality. Fresh perspective.", weakness: "Unchecked spending, missed bills, growing debt without awareness. The ostrich approach costs the most.", action: "Start with ONE thing: check your bank balance once a week. Just look. No judgment. Awareness is 80% of the battle." }
  };
  function moneyPersonality(input) {
    const counts = input.counts || { saver: 0, spender: 0, optimizer: 0, avoider: 0 };
    const type = Object.keys(counts).reduce(function(a, b) {
      return counts[a] > counts[b] ? a : b;
    });
    const profile = MONEY_PROFILES[type] || MONEY_PROFILES.saver;
    return { type, profile };
  }

  // src/engine/calculators/planning.ts
  var ALLOC_MODELS = {
    conservative: { stocks: 30, bonds: 50, cash: 20, ret: 0.055, worst: -12, color: "var(--gr)" },
    moderate: { stocks: 60, bonds: 30, cash: 10, ret: 0.075, worst: -25, color: "#5B9BFF" },
    aggressive: { stocks: 80, bonds: 15, cash: 5, ret: 0.095, worst: -38, color: "var(--or)" }
  };
  function assetAlloc(input) {
    const risk = input.risk || "moderate", years = pi(input.years) || 30;
    const assets = pf(input.assets) || 1e5, monthly = pf(input.monthly) || 500, stomach = input.stomach || "nervous";
    let m = ALLOC_MODELS[risk] || ALLOC_MODELS.moderate;
    let mismatch = false;
    if (risk === "aggressive" && stomach === "panic") {
      mismatch = true;
      m = ALLOC_MODELS.moderate;
    }
    if (years < 10 && risk === "aggressive") {
      mismatch = true;
      m = { stocks: 60, bonds: 30, cash: 10, ret: 0.075, worst: -25, color: "var(--yl)" };
    }
    const proj = [];
    let bal = assets, maxB = 0;
    for (let y = 0; y <= Math.min(years, 30); y++) {
      if (y > 0) {
        for (let mo = 0; mo < 12; mo++) bal = bal * (1 + m.ret / 12) + monthly;
      }
      maxB = Math.max(maxB, bal);
      if (y % 5 === 0 || y === years) proj.push({ yr: y, bal: Math.round(bal) });
    }
    const worstDrop = Math.round(assets * m.worst / 100);
    return { m, proj, maxB, worstDrop, mismatch };
  }
  var HSA_FAMILY_2026 = 8750;
  var HSA_SELF_2026 = 4400;
  var ROTH_IRA_2026 = 7500;
  var K401_EMPLOYEE_2026 = 24500;
  function accountType(input) {
    const income = pf(input.income) || 6e4, filing = input.filing || "married";
    const has401k = !!input.has401k, matchPct = pf(input.matchPct) || 0.04, hasHSA = !!input.hasHSA, monthly = pf(input.monthly) || 1e3;
    let bracket = income > 201775 ? 0.32 : income > 105700 ? 0.24 : income > 50400 ? 0.22 : income > 12400 ? 0.12 : 0.1;
    if (filing === "married") bracket = income > 403550 ? 0.32 : income > 211400 ? 0.24 : income > 100800 ? 0.22 : income > 24800 ? 0.12 : 0.1;
    const matchAmt = Math.round(income * matchPct / 12), maxMatch = matchAmt;
    const steps = [];
    let remaining = monthly;
    if (has401k && matchPct > 0) {
      const s1 = Math.min(remaining, Math.round(income * matchPct / 12));
      steps.push({ name: "401k to match", amt: s1, free: matchAmt, why: "FREE money \u2014 " + Math.round(matchPct * 100) + "% match", color: "var(--gr)" });
      remaining -= s1;
    }
    if (hasHSA) {
      const hsaMax = filing === "married" ? Math.round(HSA_FAMILY_2026 / 12) : Math.round(HSA_SELF_2026 / 12);
      const s2 = Math.min(remaining, hsaMax);
      steps.push({ name: "HSA max", amt: s2, free: 0, why: "Triple tax advantage \u2014 nothing beats it", color: "var(--gr)" });
      remaining -= s2;
    }
    const rothMax = Math.round(ROTH_IRA_2026 / 12);
    const s3 = Math.min(remaining, rothMax);
    if (s3 > 0) {
      steps.push({ name: "Roth IRA max", amt: s3, free: 0, why: "Tax-FREE growth, no RMDs", color: "#5B9BFF" });
      remaining -= s3;
    }
    if (has401k) {
      const k401remaining = Math.min(remaining, Math.round(K401_EMPLOYEE_2026 / 12) - Math.round(income * matchPct / 12));
      if (k401remaining > 0) {
        steps.push({ name: "401k max remaining", amt: k401remaining, free: 0, why: "Tax-deferred growth", color: "var(--or)" });
        remaining -= k401remaining;
      }
    }
    if (remaining > 0) steps.push({ name: "Taxable brokerage", amt: remaining, free: 0, why: "Flexible, no limits", color: "var(--mu)" });
    return { bracket, matchAmt, maxMatch, steps, remaining };
  }
  function currency(input) {
    const amt = pf(input.amt) || 1e3, from = input.from || "USD", to = input.to || "EUR", rates = input.rates || {};
    const inUSD = amt / (rates[from] || 1), converted = inUSD * (rates[to] || 1);
    const rate = (rates[to] || 1) / (rates[from] || 1);
    const bankFee = Math.round(amt * 0.025), afterFees = converted * (1 - 0.025);
    return { inUSD, converted, rate, bankFee, afterFees };
  }
  function emergency(input) {
    const remain = pf(input.remain) || 0, sev = pf(input.sev) || 0, sevDur = pi(input.sevDur) || 0, ef = pf(input.ef) || 0, totalExp = pf(input.totalExp) || 0;
    const essential = Math.round(totalExp * 0.6);
    const discretionary = totalExp - essential;
    const monthlyIncome = remain + sev;
    const monthlyGap = Math.max(0, essential - monthlyIncome);
    const runway = monthlyGap > 0 ? Math.round(ef / monthlyGap * 10) / 10 : 99;
    const withSeverance = sevDur > 0 ? runway + sevDur : runway;
    return { totalExp, essential, discretionary, monthlyIncome, monthlyGap, runway, withSeverance };
  }
  function windfall(input) {
    const amt = pf(input.amt) || 25e3, source = input.source || "bonus";
    const totalExp = pf(input.totalExp) || 0, efMonths = pf(input.efMonths) || 0, debts = input.debts || [];
    const taxable = source === "bonus" || source === "settlement" || source === "sale" || source === "lottery";
    const taxRate = taxable ? 0.25 : 0;
    const tax = Math.round(amt * taxRate), net = amt - tax;
    const invested20 = Math.round(net * Math.pow(1.07, 20));
    const efGap = Math.max(0, totalExp * 6 - efMonths * totalExp);
    const highDebt = debts.filter(function(d) {
      return (pf(d.rate) || 0) > 10;
    }).reduce(function(t, d) {
      return t + (pf(d.balance) || 0);
    }, 0);
    const otherDebt = debts.filter(function(d) {
      return (pf(d.rate) || 0) > 0 && (pf(d.rate) || 0) <= 10;
    }).reduce(function(t, d) {
      return t + (pf(d.balance) || 0);
    }, 0);
    const highDebtInt = debts.filter(function(d) {
      return (pf(d.rate) || 0) > 10;
    }).reduce(function(t, d) {
      return t + Math.round((pf(d.balance) || 0) * (pf(d.rate) || 0) / 100);
    }, 0);
    return { taxable, taxRate, tax, net, invested20, efGap, highDebt, otherDebt, highDebtInt };
  }

  // src/engine/calculators/fees.ts
  function advisorFee(input) {
    const port = pf(input.port) || 1e5, add = pf(input.add) || 500, grossRet = pf(input.grossRet) || 0.07, yrs = pi(input.yrs) || 25;
    const aumFee = pf(input.aumFee) || 0.01, er = pf(input.er) || 5e-3, roboFee = pf(input.roboFee) || 25e-4, flatFee = pf(input.flatFee) || 3e3;
    const loads = pf(input.loads) || 0, comm = pf(input.comm) || 0, behav = pf(input.behav) || 0.03, tlh2 = pf(input.tlh) || 5e-3;
    function simulate(label, netReturnRate, annualFixedFee, pctFeeOnAssets) {
      let bal = port * (1 - loads);
      let totalFees = port * loads;
      const snapshots = [];
      const mr = netReturnRate / 12;
      for (let y = 1; y <= yrs; y++) {
        for (let m = 0; m < 12; m++) bal = bal * (1 + mr) + add;
        const assetFee = bal * pctFeeOnAssets;
        totalFees += assetFee + annualFixedFee + comm;
        bal -= assetFee + annualFixedFee + comm;
        if (y === 5 || y === 10 || y === 15 || y === 20 || y === 25 || y === 30 || y === yrs) snapshots.push({ yr: y, val: Math.round(bal), fees: Math.round(totalFees) });
      }
      return { label, val: Math.round(bal), fees: Math.round(totalFees), snaps: snapshots };
    }
    const aum = simulate("AUM Advisor", grossRet - er + tlh2, 0, aumFee);
    const diy = simulate("Self-Managed", grossRet - er - behav, 0, 0);
    const robo = simulate("Robo-Advisor", grossRet - er + tlh2 * 0.7 - behav * 0.3, 0, roboFee);
    const flat = simulate("Flat-Fee Planner", grossRet - er + tlh2 * 0.5, flatFee, 0);
    const aumAnnual = Math.round(port * aumFee);
    const breakEven = aumFee > 0 ? Math.round(flatFee / aumFee) : 0;
    return { aum, diy, robo, flat, aumAnnual, breakEven };
  }
  var FEE_AVGS = {
    mf: { avg: 0.6, label: "Mutual Fund", taxFree: false, desc: "Actively managed diversified fund" },
    etf: { avg: 0.15, label: "ETF", taxFree: false, desc: "Passively managed index fund" },
    annuity: { avg: 2, label: "Annuity", taxFree: true, desc: "Tax-deferred accumulation + guaranteed income" },
    wl: { avg: 1.5, label: "Whole Life", taxFree: true, desc: "Tax-free cash value growth + death benefit" },
    iul: { avg: 1.8, label: "IUL", taxFree: true, desc: "Tax-free growth with market-linked upside + death benefit" },
    "401k": { avg: 0.8, label: "401(k) Plan", taxFree: false, desc: "Employer-sponsored retirement account" },
    brokerage: { avg: 0.3, label: "Brokerage Account", taxFree: false, desc: "Taxable investment account" },
    robo: { avg: 0.35, label: "Robo-Advisor", taxFree: false, desc: "Algorithm-managed automated portfolio" }
  };
  function feeScore(input) {
    const type = input.type || "mf", amt = pf(input.amt) || 1e5;
    const er = pf(input.er) || 0, aum = pf(input.aum) || 0, admin = pf(input.admin) || 0, load = pf(input.load) || 0, coi = pf(input.coi) || 0;
    const grossRet = pf(input.grossRet) || 0.07;
    const adminPct = amt > 0 ? admin / amt : 0;
    const totalPct = er + aum + adminPct + coi;
    const totalAnnual = Math.round(amt * totalPct + admin);
    const loadCost = Math.round(amt * load);
    const info = FEE_AVGS[type] || { avg: 0.5, label: "Product", taxFree: false, desc: "" };
    const diff = totalPct * 100 - info.avg;
    const diffDollar = Math.round(diff / 100 * amt);
    const noFee10 = amt * Math.pow(1 + grossRet, 10), withFee10 = amt * Math.pow(1 + grossRet - totalPct, 10);
    const noFee20 = amt * Math.pow(1 + grossRet, 20), withFee20 = amt * Math.pow(1 + grossRet - totalPct, 20);
    const noFee30 = amt * Math.pow(1 + grossRet, 30), withFee30 = amt * Math.pow(1 + grossRet - totalPct, 30);
    const cost10 = Math.round(noFee10 - withFee10), cost20 = Math.round(noFee20 - withFee20), cost30 = Math.round(noFee30 - withFee30);
    const taxRate = 0.15;
    const taxFreeNet30 = Math.round(withFee30);
    const taxableNet30 = Math.round(withFee30 - (withFee30 - amt) * taxRate);
    const taxAdj = info.taxFree ? taxFreeNet30 : taxableNet30;
    return { adminPct, totalPct, totalAnnual, loadCost, info, diff, diffDollar, noFee10, withFee10, noFee20, withFee20, noFee30, withFee30, cost10, cost20, cost30, taxFreeNet30, taxableNet30, taxAdj };
  }

  // src/engine/calculators/simple.ts
  function investReturn(input) {
    const init = pf(input.init) || 0, monthly = pf(input.monthly) || 0;
    const ret = pf(input.ret) || 0.07, years = pi(input.years) || 25, infl = pf(input.infl) || 0.03;
    const mr = ret / 12;
    let bal = init, contributed = init;
    const milestones = [];
    for (let y = 1; y <= years; y++) {
      for (let m = 0; m < 12; m++) {
      ...
      if (y % 5 === 0 || y === years || y === 1) milestones.push({ yr: y, bal: Math.round(bal), contr: Math.round(contributed) });
    }
    const growth = Math.round(bal - contributed), maxBal = bal;
    const growthPct = contributed > 0 ? Math.round(growth / contributed * 100) : 0;
    let savBal = init;
    for (let y2 = 0; y2 < years; y2++) savBal = (savBal + monthly * 12) * 1.005;
    const advantage = Math.round(bal - savBal);
    const realBal = Math.round(bal / Math.pow(1 + infl, years));
    const realGrowth = Math.round(realBal - contributed);
    return { bal, contributed, growth, maxBal, growthPct, milestones, savBal, advantage, realBal, realGrowth };
  }
  function costOfWaiting(input) {
    const age = pi(input.age) || 30, amt = pf(input.amt) || 500;
    const ret = pf(input.ret) || 0.07, retAge = pi(input.retAge) || 65;
    const mr = ret / 12;
    const delays = [0, 1, 2, 3, 5, 10];
    const results = delays.map(function(d) {
      const months = Math.max(0, (retAge - age - d) * 12);
      let bal = 0;
      for (let m = 0; m < months; m++) bal = bal * (1 + mr) + amt;
      return { delay: d, val: Math.round(bal), contributed: Math.round(amt * months) };
    });
    const maxVal = results[0].val || 1;
    const dailyCost = results[1] ? Math.round((results[0].val - results[1].val) / 365) : 0;
    return { results, maxVal, dailyCost };
  }
  var DCA_CRASH = [0.12, -0.05, 0.15, 0.08, -0.4, 0.3, 0.15, 0.1, 0.05, 0.12, -0.08, 0.2, 0.1, 0.06, 0.15, -0.02, 0.18, 0.08, 0.12, 0.07];
  var DCA_BULL = [0.15, 0.12, 0.08, 0.2, 0.1, 0.14, 0.06, 0.18, 0.09, 0.11, 0.13, 0.07, 0.16, 0.1, 0.12, 0.08, 0.15, 0.11, 0.09, 0.13];
  var DCA_AVG = [0.1, -0.02, 0.15, 0.05, 0.12, -0.08, 0.2, 0.08, 0.06, 0.14, -0.05, 0.18, 0.03, 0.12, 0.07, 0.15, -0.03, 0.1, 0.08, 0.11];
  function dca(input) {
    const monthly = pf(input.monthly) || 500, years = pi(input.years) || 20;
    const scenario = input.scenario || "average", lump = pf(input.lump) || 3e4;
    const annualReturns = scenario === "crash" ? DCA_CRASH.slice() : scenario === "bull" ? DCA_BULL.slice() : DCA_AVG.slice();
    while (annualReturns.length < years) annualReturns.push(annualReturns[annualReturns.length % annualReturns.length]);
    let bal = 0, invested = 0, sharePrice = 100, shares = 0;
    const milestones = [];
    for (let y = 0; y < years; y++) {
      const ret = annualReturns[y % annualReturns.length];
      for (let m = 0; m < 12; m++) {
        sharePrice = sharePrice * (1 + ret / 12);
        const newShares = monthly / sharePrice;
        shares += newShares;
        invested += monthly;
      }
      bal = shares * sharePrice;
      if (y % 3 === 0 || y === years - 1) milestones.push({ yr: y + 1, bal: Math.round(bal), inv: Math.round(invested), price: Math.round(sharePrice * 100) / 100 });
    }
    const growth = Math.round(bal - invested), avgCost = invested / shares;
    let lumpBal = lump;
    for (let yy = 0; yy < years; yy++) lumpBal = lumpBal * (1 + 0.07);
    return { annualReturns, bal, invested, growth, avgCost, milestones, lumpBal };
  }
  function dividend(input) {
    const init = pf(input.init) || 5e4, monthly = pf(input.monthly) || 500;
    const divYield = pf(input.divYield) || 0.02, growth = pf(input.growth) || 0.05;
    const years = pi(input.years) || 20, acct = input.acct || "taxable";
    const taxRate = acct === "taxable" ? 0.15 : 0;
    let rBal = init, rDiv1 = 0, rDivTotal = 0;
    for (let y = 0; y < years; y++) {
      const annualDiv = rBal * divYield;
      if (y === 0) rDiv1 = annualDiv;
      const afterTax = annualDiv * (1 - taxRate);
      rBal = (rBal + afterTax + monthly * 12) * (1 + growth);
      rDivTotal += annualDiv;
    }
    let cBal = init, cDivTotal = 0;
    for (let y2 = 0; y2 < years; y2++) {
      const annualDiv2 = cBal * divYield;
      cDivTotal += annualDiv2;
      cBal = (cBal + monthly * 12) * (1 + growth);
    }
    const advantage = Math.round(rBal - cBal), lastYearDiv = Math.round(rBal * divYield);
    return { rBal, rDiv1, rDivTotal, cBal, cDivTotal, advantage, lastYearDiv };
  }
  function efFund(input) {
    const exp = pf(input.exp) || 0, savings = pf(input.savings) || 0;
    const stability = input.stability || "single", savRate = pf(input.savRate) || 300;
    const targetMonths = stability === "stable" ? 3 : stability === "variable" ? 9 : stability === "irregular" ? 12 : 6;
    const target = exp * targetMonths, gap = Math.max(0, target - savings);
    const currentMonths = exp > 0 ? savings / exp : 0;
    const pct = target > 0 ? Math.min(100, Math.round(savings / target * 100)) : 100;
    const monthsToFull = savRate > 0 && gap > 0 ? Math.ceil(gap / savRate) : 0;
    const hysaRate = 0.045, annualInterest = Math.round(target * hysaRate);
    const checkingRate = 1e-3, hysaAdvantage = Math.round(target * (hysaRate - checkingRate));
    return { targetMonths, target, gap, currentMonths, pct, monthsToFull, hysaRate, annualInterest, checkingRate, hysaAdvantage };
  }
  function netWorthCalc(input) {
    const checking = pf(input.checking) || 0, invest = pf(input.invest) || 0, ret = pf(input.ret) || 0, home = pf(input.home) || 0;
    const mortgage = pf(input.mortgage) || 0, otherDebt = pf(input.otherDebt) || 0, age = pf(input.age) || 0;
    const assets = checking + invest + ret + home, debts = mortgage + otherDebt, nw = assets - debts;
    const avgNW = age < 30 ? 1e4 : age < 40 ? 8e4 : age < 50 ? 2e5 : age < 60 ? 4e5 : 7e5;
    const vsAvg = nw - avgNW;
    return { assets, debts, nw, avgNW, vsAvg };
  }
  function savConsistency(input) {
    const planned = pf(input.planned) || 500, actual = pf(input.actual) || 0;
    ...
    for (let m = 0; m < 360; m++) {
      projPlanned = (projPlanned + planned) * (1 + 0.07 / 12);
      projActual = (projActual + actual) * (1 + 0.07 / 12);
    }
    const gap = Math.round(projPlanned - projActual), monthlyShortfall = planned - actual;
    return { pct, grade, projPlanned, projActual, gap, monthlyShortfall };
  }
  function canIAfford(input) {
    const amt = pf(input.amt) || 0, netCF = pf(input.netCF) || 500, inc = pf(input.inc) || 0;
    const remaining = Math.max(0, netCF);
    const hourlyWage = Math.round(inc / 168);
    const hours = hourlyWage > 0 ? Math.round(amt / hourlyWage * 10) / 10 : 0;
    const invested10 = Math.round(amt * Math.pow(1.07, 10));
    const invested30 = Math.round(amt * Math.pow(1.07, 30));
    const pctBudget = remaining > 0 ? Math.round(amt / remaining * 100) : 999;
    return { remaining, hourlyWage, hours, invested10, invested30, pctBudget };
  }

  // src/engine/index.ts
  var ENGINE_VERSION = "v51";
