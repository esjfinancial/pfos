...
var PFOSEngine = (() => {
  ...
  function _ibcPolicyDefaults(type) {
    if (type === "wl") {
      return {
        ...
        loanRate: 0.05,
        cvEfficiencyYr: [null, 0.55, 0.75, 0.75, 0.9, 0.9, 1, 1, 1, 1, 1]
      };
    }
    return {
      type: "iul",
  ...
  function simulateIBCDebtCascade(debts, budget, opts = {}) {
    ...
    const maxMonths = opts.maxMonths != null ? opts.maxMonths : 720;
    ...
    const ibcParams = Object.assign({}, _ibcPolicyDefaults("wl"), { monthlyPremium: budget }, opts.policyParams || {});
    ...
    let vehicle = 0, totalDebtInt = 0, totalLoanInt = 0, month = 0;
    ...
    while (month < maxMonths) {
      ...
      for (let _i = 0; _i < ds.length; _i++) {
        ...
        if (d.paid) continue;
        const borrowable = ltvCap * vehicle - totalLoanBalance;
        if (borrowable >= d.bal) {
      ...
      if (month % 12 === 0) {
        ...
        const annualCredit = _ibcWLAnnualCredit(ibcParams, policyYear, lastAnnivCV, 0);
        const efficiency = _ibcYearFactor(ibcParams.cvEfficiencyYr, policyYear, 1);
        const stuckPremiumCV = premiumSinceAnniv * efficiency;
        const growthOnExistingCV = annualCredit.breakdown.guaranteedGrowth + annualCredit.breakdown.dividend;
        lastAnnivCV = lastAnnivCV + stuckPremiumCV + growthOnExistingCV;
        premiumSinceAnniv = 0;
        vehicle = lastAnnivCV;
      }
      ...
      if (!origLeft && totalLoanBalance <= 0) break;
    }
    ...
    for (let _t = 0; _t < trailMonths; _t++) {
      ...
      if (month % 12 === 0) {
        const pyT = month / 12;
        const acT = _ibcWLAnnualCredit(ibcParams, pyT, lastAnnivCV, 0);
        const effT = _ibcYearFactor(ibcParams.cvEfficiencyYr, pyT, 1);
        lastAnnivCV = lastAnnivCV + premiumSinceAnniv * effT + acT.breakdown.guaranteedGrowth + acT.breakdown.dividend;
        premiumSinceAnniv = 0;
        vehicle = lastAnnivCV;
      }
...
All debts are cleared by month ${r.months} (${yr(r.months)} yr). The policy loans are then repaid with the freed minimums and fully cleared by month ${r.fullFreeMonth} (${yr(r.fullFreeMonth)} yr), after which those minimums cascade toward your other goals.`;
  ...
  function computeCalcs(S) {
    ...
    let fiNumber = 0;
    for (let _fi = 0; _fi < retYearsInRetirement; _fi++) {
      fiNumber += retInc * 12 * Math.pow(1 + retInflation, _fi);
    }
